...
export type SelectedShoppingListItemsStateModel =
  EntityState<ShoppingListItemModel>;

export const selectedShoppingListItemsAdapter =
  createEntityAdapter<ShoppingListItemModel>({
    sortComparer: ShoppingListsService.sortShoppingListItems,
  });

const initialState: SelectedShoppingListItemsStateModel =
  selectedShoppingListItemsAdapter.getInitialState();
...
export const selectedShoppingListItemsFeature = createFeature({
  ...
  reducer: createReducer(
    ...
    on(
      SelectedShoppingListItemsApiActions.getItemsSuccess,
      (state, { items }) => {
    ...
    on(
      SelectedShoppingListItemsApiActions.updateItemsSuccess,
      (state, { dto }) => {
        return selectedShoppingListItemsAdapter.upsertMany(
          [...dto.createdShoppingListItems, ...dto.updatedShoppingListItems],
          selectedShoppingListItemsAdapter.removeMany(
            dto.removedShoppingListItemsIds,
            state
          )
        );
      }
