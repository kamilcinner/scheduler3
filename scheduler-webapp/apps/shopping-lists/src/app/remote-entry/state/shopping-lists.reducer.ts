...
const initialState: ShoppingListsStateModel =
...
export const shoppingListsFeature = createFeature({
  ...
  reducer: createReducer(
    ...
    on(
      ShoppingListsApiActions.createSuccess,
      (state, { createdShoppingList }) => {
    ...
    on(
      ShoppingListsApiActions.updateSuccess,
      (state, { updatedShoppingList }) => {
        return shoppingListsAdapter.upsertOne(updatedShoppingList, state);
      }
    ),

    on(
      ShoppingListsApiActions.removeSuccess,
      (state, { removedShoppingList }) => {
